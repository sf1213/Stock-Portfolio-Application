...
public class PortfolioTest {
  ...
  /**
   * Test Add Stock to an exsting Portfolio.
   */
  @Test
  public void testAddStock() {
    InputStream in = new ByteArrayInputStream(("3 retirement 1 AAPL # 9 1 MSFT").getBytes());
    ByteArrayOutputStream bytes = new ByteArrayOutputStream();
    PrintStream out = new PrintStream(bytes);
    view = new PortfolioView(out);
    controller = new PortfoliosController(in, model, view);
    ...
    String outExpected = "MSFT AAPL";

    assertEquals(model.getStockNames("retirement"), outExpected);
  }

  /**
   * Test Buy Stock by specifying actual amount, and date without Commission Fee.
   */
  @Test
  public void testBuyStockNoFee() {
    InputStream in = new ByteArrayInputStream(("3 retirement 1 AAPL MSFT # 4 1 1 1 2018-03-12 N "
            + "100 4").getBytes());
    ByteArrayOutputStream bytes = new ByteArrayOutputStream();
    PrintStream out = new PrintStream(bytes);
    view = new PortfolioView(out);
    ...
    String outExpected =
            "1: StockTickerSymbol: MSFT, ShareOwned: 100.000000, BuyPrice: 96.77, Cost: 9677.00, "
                    + "Commission: 0.00, BuyDate: 03/12/18\n\n";

    assertEquals(model.examine("retirement"), outExpected);
  }

  /**
   * Test Buy Stock by specifying actual amount, and date with Commission Fee.
   */
  @Test
  public void testBuyStockFee() {
    InputStream in = new ByteArrayInputStream(("3 retirement 1 AAPL MSFT # 4 1 1 1 2018-03-12 Y 10 "
            + "100 4").getBytes());
    ByteArrayOutputStream bytes = new ByteArrayOutputStream();
    PrintStream out = new PrintStream(bytes);
    view = new PortfolioView(out);
  ...
  /**
   * Test Buy stocks by specifying an amount, date and Weight distribution without Commission Fees.
   */
  @Test
  public void testApplyStrategyNotEqualWeightNoFee() {
    InputStream in = new ByteArrayInputStream(("3 retirement 1 AAPL MSFT # 7 retirement "
            + "1 N 0.3 0.7 500 N 5 "
            + "2018-01-02 Y 2018-01-03 N").getBytes());
    ByteArrayOutputStream bytes = new ByteArrayOutputStream();
    PrintStream out = new PrintStream(bytes);
    view = new PortfolioView(out);
    controller = new PortfoliosController(in, model, view);
    ...
    String outExpected = "1: StockTickerSymbol: MSFT, ShareOwned: 1.745201, BuyPrice: 85.95, "
            + "Cost: 150.00, Commission: 0.00, BuyDate: 01/02/18\n\n"
            + "2: StockTickerSymbol: AAPL, ShareOwned: 2.031812, BuyPrice: 172.26, "
            + "Cost: 350.00, Commission: 0.00, BuyDate: 01/02/18\n\n";

    assertEquals(model.examine("retirement"), outExpected);
  }

  /**
   * Test Buy stocks by specifying an amount, date and Weight distribution with Commission Fees.
   */
  @Test
  public void testApplyStrategyNotEqualWeightFee() {
    InputStream in = new ByteArrayInputStream(("3 retirement 1 AAPL MSFT # 7 retirement "
            + "1 N 0.3 0.7 500 Y 10 5 "
            + "2018-01-02 Y 2018-01-03 N").getBytes());
    ByteArrayOutputStream bytes = new ByteArrayOutputStream();
    PrintStream out = new PrintStream(bytes);
    view = new PortfolioView(out);
  ...
  /**
   * Test Save portfolio (retirement) and Retrieve portfolio(song) from files(./retirement.json).
   */
  @Test
  public void testSaveRetrievePortfolio() {

    //create a "retirement" portfolio, save it to file,
    //then create a "song" portfolio, retrieve the saved file "retirement.json" to "song"
    InputStream in = new ByteArrayInputStream(("3 retirement 1 AAPL MSFT # 7 retirement 1 "
            + "N 0.3 0.7 500 Y 10 5 "
            + "2018-01-02 Y 2018-01-03 N 8 1 3 song 2 ./retirement.json").getBytes());
    ByteArrayOutputStream bytes = new ByteArrayOutputStream();
    PrintStream out = new PrintStream(bytes);
    view = new PortfolioView(out);
    controller = new PortfoliosController(in, model, view);
    ...
    //Data format and contents in "song" and "retirement" should be Same
    assertEquals(model.examine("song").equals(model.examine("retirement")), true);
  }

  /**
   * Test Save strategy (on retirement) and Retrieve strategy(to song) from files(saved retirement).
   */
  @Test
  public void testSaveRetrieveStrategy() {

    //retrieve a strategy json file from user
    InputStream in = new ByteArrayInputStream(("3 retirement 1 AAPL MSFT # 7 retirement "
            + "1 N 0.3 0.7 500 Y 10 5 "
            + "2018-01-02 Y 2018-01-03 Y  3 song 1 AAPL MSFT # "
            + "7 song 2 ./Strategy-2018-01-02-2018-01-03-5.json").getBytes());
    ByteArrayOutputStream bytes = new ByteArrayOutputStream();
    PrintStream out = new PrintStream(bytes);
    view = new PortfolioView(out);
    controller = new PortfoliosController(in, model, view);
    ...
    //the correct format and contents should be like this
    String outExpected = "1: StockTickerSymbol: MSFT, ShareOwned: 1.745201, BuyPrice: 85.95, "
            + "Cost: 153.00, Commission: 3.00, BuyDate: 01/02/18\n\n"
            + "2: StockTickerSymbol: AAPL, ShareOwned: 2.031812, BuyPrice: 172.26, "
            + "Cost: 357.00, Commission: 7.00, BuyDate: 01/02/18\n\n";

    assertEquals(model.examine("song"), outExpected);
  }
